...
Процедура НайтиОбъектИнфобазы(Команда)
	...
	ПараметрыПоиска.Вставить("ИсходнаяСтрокаПоиска", ТекущиеДанные.ЗначениеВFeature);
	ПараметрыПоиска.Вставить("ИмяСправочника", ОбщегоНазначенияСервер.ПолучитьИмяСправочникаПоСсылке(ТекущиеДанные.ЗначениеВAgiler));
