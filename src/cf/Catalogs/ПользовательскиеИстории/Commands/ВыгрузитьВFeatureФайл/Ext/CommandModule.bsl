...
Процедура ВыгрузитьИзФормыЭлемента(ТекущийЭлемент)
	ИмяFeatureФайла = 
		ОбщегоНазначениеСервер.ПолучитьЗначениеРеквизита(
			"Справочник", "ПользовательскиеИстории", ТекущийЭлемент, "ИмяFeatureФайла"
		);
	
	Файл = Новый Файл(ИмяFeatureФайла);
	
	Файл.НачатьПроверкуСуществования(
		Новый ОписаниеОповещения("ПроверкаСуществованияFeatureФайлаЗавершение", ЭтотОбъект, ТекущийЭлемент)
	);
КонецПроцедуры
...
Процедура ПроверкаСуществованияFeatureФайлаЗавершение(Существует, ТекущийЭлемент) Экспорт
	...
	Если НетОшибок Тогда
		...
		ТекстовыйДокумент.НачатьЗапись(
			...
			ПутьКFeatureФайлу, КодировкаТекста.UTF8
		);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьШаблон(ТекущийЭлемент, ШаблонФичи, НетОшибок)
	...
	ШаблонФичи = СтрЗаменить(ШаблонФичи, "#Код", XMLСтрока(ВыборкаДетальныеЗаписи.Код));
