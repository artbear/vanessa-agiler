...
Процедура КаталогПроектаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогВыбораКаталога.Показать(
		Новый ОписаниеОповещения("ВыборКаталогаПроектаЗавершение", ЭтотОбъект)
	);
КонецПроцедуры
...
Процедура ВыборКаталогаПроектаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Сообщение.Текст = "В качестве каталога проекта возможно выбрать только каталог с именем """
			 + ИмяКаталогаПроекта + """";
...
Процедура КаталогПроектаОткрытие(Элемент, СтандартнаяОбработка)
	...
	НачатьЗапускПриложения(
		Новый ОписаниеОповещения("ОткрытиеКаталогаПроектаЗавершение", ЭтотОбъект), 
